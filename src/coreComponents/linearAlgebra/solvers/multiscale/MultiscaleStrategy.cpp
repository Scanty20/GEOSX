...
#include "linearAlgebra/solvers/multiscale/MsRSBStrategy.hpp"
                                           ...
                                           LinearSolverParameters::Multiscale params )
{
  switch( params.type )
  {
    case LinearSolverParameters::Multiscale::Type::msrsb:
      return std::make_unique< MsRSBStrategy< LAI > >( std::move( name ), std::move( params ) );
    default:
      GEOSX_ERROR( "Unsupported interpolation type: " << params.type );
  }
