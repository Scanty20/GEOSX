...
HYPRE_Int getHypreILUType( LinearSolverParameters::PrecondType const type )
{
  static std::map< LinearSolverParameters::PrecondType, HYPRE_Int > const typeMap =
  {
    { LinearSolverParameters::PrecondType::iluk, 0 },
    { LinearSolverParameters::PrecondType::ilut, 1 },
  };
                            ...
                            array1d< HYPRE_ParVector > & nullSpacePointer )
{
  ...
  else
  {
    ...
    localIndex const numRotations = hypre::toInt( nearNullKernel.size() - dim );
                ...
                HyprePrecWrapper & precond )
{
  ...
  GEOSX_LAI_CHECK_ERROR( HYPRE_BoomerAMGSetPrintLevel( precond.ptr, hypre::toInt( params.logLevel ) ) );;
  ...
  GEOSX_LAI_CHECK_ERROR( HYPRE_BoomerAMGSetMaxLevels( precond.ptr, hypre::toInt( params.amg.maxLevels ) ) );
  ...
  else
  {
    ...
    if( params.amg.smootherType == LinearSolverParameters::AMG::SmootherType::chebyshev )
    {
      ...
      {
        GEOSX_LAI_CHECK_ERROR( HYPRE_BoomerAMGSetChebyOrder( precond.ptr, hypre::toInt( params.amg.numSweeps ) ) );
      }
    }
  }
  ...
  switch( params.amg.preOrPostSmoothing )
  {
    ...
    {
      GEOSX_LAI_CHECK_ERROR( HYPRE_BoomerAMGSetNumSweeps( precond.ptr, hypre::toInt( params.amg.numSweeps ) ) );
      break;
    }
    case LinearSolverParameters::AMG::PreOrPost::pre:
    {
      GEOSX_LAI_CHECK_ERROR( HYPRE_BoomerAMGSetCycleNumSweeps( precond.ptr, hypre::toInt( params.amg.numSweeps ), 1 ) );
    ...
    case LinearSolverParameters::AMG::PreOrPost::post:
    {
      ...
      GEOSX_LAI_CHECK_ERROR( HYPRE_BoomerAMGSetCycleNumSweeps( precond.ptr, hypre::toInt( params.amg.numSweeps ), 2 ) );
      break;
    }
    default:
    {
                ...
                HyprePrecWrapper & precond )
{
  ...
  {
    GEOSX_LAI_CHECK_ERROR( HYPRE_ILUSetLevelOfFill( precond.ptr, hypre::toInt( params.ifact.fill ) ) );
  }
  if( params.ifact.threshold >= 0 && params.preconditionerType == LinearSolverParameters::PrecondType::ilut )
  {
                ...
                HypreMGRData & mgrData )
{
  ...
  GEOSX_LAI_CHECK_ERROR( HYPRE_MGRSetPrintLevel( precond.ptr, hypre::toInt( params.logLevel ) ) );
  ...
  if( params.preconditionerType == LinearSolverParameters::PrecondType::mgr && params.mgr.separateComponents )
  {
...
{
  if( m_params.preconditionerType == LinearSolverParameters::PrecondType::amg &&
...
void HyprePreconditioner::create( DofManager const * const dofManager )
{
  ...
  {
    case LinearSolverParameters::PrecondType::none:
    {
    ...
    case LinearSolverParameters::PrecondType::jacobi:
    {
    ...
    case LinearSolverParameters::PrecondType::amg:
    {
      createAMG( m_params, *m_nullSpace, *m_precond );
      break;
    }
    case LinearSolverParameters::PrecondType::mgr:
    {
      m_mgrData = std::make_unique< HypreMGRData >();
      createMGR( m_params, dofManager, *m_precond, *m_mgrData );
      break;
    }
    case LinearSolverParameters::PrecondType::iluk:
    case LinearSolverParameters::PrecondType::ilut:
    {
      createILU( m_params, *m_precond );
      break;
    }
    case LinearSolverParameters::PrecondType::direct:
    {
...
HypreMatrix const & HyprePreconditioner::setupSeparateComponent( HypreMatrix const & mat )
{
  Stopwatch timer( m_componentFilterTime );
  if( m_params.preconditionerType == LinearSolverParameters::PrecondType::mgr && m_params.mgr.separateComponents )
  {
    ...
    LAIHelperFunctions::separateComponentFilter( Auu, m_separateComponentMatrix, m_params.dofsPerNode );
  }
  else if( m_params.preconditionerType == LinearSolverParameters::PrecondType::amg && m_params.amg.separateComponents )
  {
    LAIHelperFunctions::separateComponentFilter( mat, m_separateComponentMatrix, m_params.dofsPerNode );
    return m_separateComponentMatrix;
  }
  return mat;
}
...
void HyprePreconditioner::setup( Matrix const & mat )
{
  ...
  HypreMatrix const & precondMat = setupSeparateComponent( mat );
  create( mat.dofManager() );
  ...
  // To be able to use Hypre preconditioner (e.g., BoomerAMG) we need to disable floating point exceptions
  {
    ...
    {
      m_mgrData->mechSolver.setup( m_mgrData->mechSolver.ptr, m_separateComponentMatrix.unwrapped(), nullptr, nullptr );
    }
  }
}
