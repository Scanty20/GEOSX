                             ...
                             real64 * const values ) const
{
  ...
  if( m_targetRank < 0 || m_targetRank == rank )
  {
    ...
    // WTF: We have to handle two cases differently because hypre uses two different struct members (!)
    //      to store the column indices depending on how we obtained the local matrix (this is fucked).
    if( m_targetRank < 0 )
    {
    ...
    else
    {
      ...
      // Sort the values by column index after copying (some solvers expect this, but not guaranteed)
  ...
  if( m_targetRank >= 0 )
  {
    ...
    if( m_subComm != MPI_COMM_NULL )
    {
      ...
      if( rank == m_targetRank )
      {
        ...
        localVector = hypre_SeqVectorCreate( vec.globalSize() );
